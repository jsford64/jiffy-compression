...
SCANTYPES = { "RANGE":0,         "RANGE2":1, 
              "SIGNAL":2,        "SIGNAL2":3, 
              "REFLECTIVITY":4, "REFLECTIVITY2":5, 
              "NEARIR":6
            }   
...
class Stream:
    ...
    scansPerFrame:  
        ...
        etc.), returned by the LiDAR in one sweep. The group of scans returned 
        in one sweep is called a frame. Ignored on decode, since the number of
        scans per frame is encoded in the stream.
    ...
    framesPerGroup:
        ...
        I scans). Default is 10. Ignored on decode, since the number of frames
        per group is encoded in the stream.

        If framesPerGroup == 0, no groups are encoded. All scans in the frame will 
        be coded according to mode.

        If framesPerGroup > 0, a group is encoded. All scans in the first frame 
        of each group will be intra-coded.
        All other frames in the group will be coded as temporal or spatial scans 
        if mode=ADAPTIVE, or as intra-coded scans if mode=ISCAN.

        framesPerGroup must be a positive integer.

        Groups of frames are used to improve random access and potentially aid in 
        error recovery when decoding.
    ...
    byteStream:
        ...
        decoded frames.
        Stream defaults to using an empty bytes object (b'').
    ...
    Encoded stream format:
        ...
        ------------------
            ...
            ---------------------------------------------------------
            "JFFY"          ASCII,          4           File 'magic number' identifier
            version         uint8,          1           Version of the Jiffy file format
    ...
    def readHeader(self):
        '''
        Read the Jiffy header from the byteStream.
        '''
    ...
    def decode(self):
        '''
        Decode a frame from the byteStream.
